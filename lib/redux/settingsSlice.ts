...
export const selectThemeColor = (state: RootState) => (state.settings as Settings).themeColor;

export const selectFormToShow = (state: RootState) => (state.settings as Settings).formToShow;
export const selectShowByForm = (form: ShowForm) => (state: RootState) =>
  (state.settings as Settings).formToShow[form];

export const selectFormToHeading = (state: RootState) =>
  (state.settings as Settings).formToHeading;
export const selectHeadingByForm = (form: ShowForm) => (state: RootState) =>
  (state.settings as Settings).formToHeading[form];

export const selectFormsOrder = (state: RootState) => (state.settings as Settings).formsOrder;
export const selectIsFirstForm = (form: ShowForm) => (state: RootState) =>
  (state.settings as Settings).formsOrder[0] === form;
export const selectIsLastForm = (form: ShowForm) => (state: RootState) =>
  ((state.settings as Settings).formsOrder[(state.settings as Settings).formsOrder.length - 1] === form);

export const selectShowBulletPoints =
  (form: FormWithBulletPoints) => (state: RootState) =>
    (state.settings as Settings).showBulletPoints[form];
